...
import Base.GMP: ClongMax, CulongMax, CdoubleMax, MPZ, _MPZ, _Z
...
function convert(::Type{BigFloat}, x::BigInt)
    z = BigFloat()
    ccall((:mpfr_set_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &MPZ(x), ROUNDING_MODE[end])
    return z
end
...
function unsafe_cast(::Type{BigInt}, x::BigFloat, ri::Cint)
    ...
    ccall((:mpfr_get_z, :libmpfr), Int32, (Ptr{_MPZ}, Ptr{BigFloat}, Int32),
          &_Z, &x, ri)
    BigInt(_Z)
end
...
for (fJ, fC) in ((:+,:add), (:*,:mul))
    @eval begin
        ...
        function ($fJ)(x::BigFloat, c::BigInt)
            z = BigFloat()
            ccall(($(string(:mpfr_,fC,:_z)), :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &MPZ(c), ROUNDING_MODE[end])
            return z
        end
...
function -(c::BigInt, x::BigFloat)
    z = BigFloat()
    ccall((:mpfr_z_sub, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}, Ptr{BigFloat}, Int32), &z, &MPZ(c), &x, ROUNDING_MODE[end])
    return z
end
...
function div(x::BigFloat, c::BigInt)
    z = BigFloat()
    ccall((:mpfr_div_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &MPZ(c), to_mpfr(RoundToZero))
    ccall((:mpfr_trunc, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}), &z, &z)
    return z
end
...
function ^(x::BigFloat, y::BigInt)
    z = BigFloat()
    ccall((:mpfr_pow_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &MPZ(y), ROUNDING_MODE[end])
    return z
end
...
function cmp(x::BigFloat, y::BigInt)
    ...
    ccall((:mpfr_cmp_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}), &x, &MPZ(y))
end
