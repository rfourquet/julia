...
function sqrmod!(f::GF2X, m::GF2X)
    ...
    c = foldl(GF2X(0), filter(j->coeff(f, j)==1, 0:d2)) do c, i; setbit(c.z, 2i); c end
    d2 < degree(f) ? # to avoid spurious TypeError with an empty range
        foldl(xor!, c, sqrs[i] for i in filter(j->coeff(f, j)==1, d2+1:degree(f))) : c
end
...
Compute the jump polynomial for `steps` steps for dSFMT, by default with exponent 19937).
Note that `steps` should be less than the period (e.g. `steps ≪ 2^19937-1`)."""
function calc_jump(steps::Integer,
                   charpoly::GF2X=get!(()->GF2X(Poly19937), JumpPolys, -1))
