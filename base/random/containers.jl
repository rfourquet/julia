...
rand!(A::AbstractDict{K,V}, dist::Distribution{Pair} = Distribution(Pair, K, V)) where {K,V} =
...
function _rand!(rng::AbstractRNG, A::AbstractDict{K,V}, n::Integer, sp::Sampler) where {K,V}
    empty!(A)
    while length(A) < n
        push!(A, rand(rng, sp))
    end
